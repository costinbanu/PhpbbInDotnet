    ...
    class AdminForumService : MultilingualServiceBase, IAdminForumService
    {
        ...
        public async Task<(string Message, bool? IsSuccess)> ManageForumsAsync(UpsertForumDto dto, int adminUserId, bool isRoot)
        {
            ...
            try
            {
                if (isRoot)
                {
                    ...
                    return (string.Format(TranslationProvider.Admin[lang, "FORUM_UPDATED_SUCCESSFULLY_FORMAT"], _config.GetObject<string>("ForumName")), true);
                }
                ...
                {
                    return (TranslationProvider.Admin[lang, "INVALID_FORUM_NAME"], false);
                }
                if ((dto.ForumId ?? 0) > 0 && actual == null)
                {
                    return (string.Format(TranslationProvider.Admin[lang, "FORUM_DOESNT_EXIST_FORMAT"], dto.ForumId), false);
                }
                ...
                return (string.Format(TranslationProvider.Admin[lang, "FORUM_UPDATED_SUCCESSFULLY_FORMAT"], actual.ForumName), true);
            }
            catch (Exception ex)
            {
        ...
        public async Task<(string Message, bool? IsSuccess)> DeleteForum(int forumId, int adminUserId)
        {
            ...
            try
            {
                ...
                if (forum == null)
                {
                    return (string.Format(TranslationProvider.Admin[lang, "FORUM_DOESNT_EXIST_FORMAT"], forumId), false);
                }
                if (await _context.PhpbbForums.AsNoTracking().CountAsync(x => x.ParentId == forumId) > 0)
                {
                    return (string.Format(TranslationProvider.Admin[lang, "CANT_DELETE_HAS_CHILDREN_FORMAT"], forum.ForumName), false);
                }
                if (await _context.PhpbbTopics.AsNoTracking().CountAsync(x => x.ForumId == forumId) > 0)
                {
                    return (string.Format(TranslationProvider.Admin[lang, "CANT_DELETE_HAS_TOPICS_FORMAT"], forum.ForumName), false);
                }
                ...
                return (string.Format(TranslationProvider.Admin[lang, "FORUM_DELETED_SUCCESSFULLY_FORMAT"], forum.ForumName), true);
            }
            catch (Exception ex)
            {
                var id = Utils.HandleError(ex);
