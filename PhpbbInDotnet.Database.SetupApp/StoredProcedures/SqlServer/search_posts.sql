/****** Object:  StoredProcedure [dbo].[search_posts]    Script Date: 03.07.2023 21:11:35 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @searchable_forums_table TABLE(forum_id int INDEX ix_forum_id CLUSTERED);
	INSERT INTO @searchable_forums_table
	SELECT value FROM string_split(@searchable_forums, ',');

	DECLARE @filtered_posts TABLE(post_id int INDEX ix_post_id CLUSTERED);
	...
	DECLARE @posts TABLE (forum_id int not null, 
						  topic_id int not null, 
						  post_id int not null INDEX ix_post_id CLUSTERED, 
	...
	BEGIN
		...
		SELECT p.forum_id,
		  ...
		  FROM phpbb_posts p
		  JOIN @filtered_posts fp on fp.post_id = p.post_id
		 WHERE CONTAINS(p.post_text, @search_text)
		 ORDER BY p.post_time DESC
		OFFSET @skip ROWS FETCH NEXT @take ROWS ONLY;
	END
