...
public class RateLimitingMiddleware(IConfiguration configuration, IAnonymousSessionCounter anonymousSessionCounter, IDistributedCache cache) : IMiddleware
{
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        if (await ShouldRateLimitPage(context) && options.ShouldRateLimit && anonymousSessionCounter.ShouldRateLimit(userAgent, ip, sessionId, options.Threshold, options.TimeWindow))
        {
