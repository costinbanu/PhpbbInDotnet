CREATE DEFINER=`root`@`localhost` PROCEDURE `get_own_topics`(user_id_parm int, skip_parm int, take_parm int, excluded_forums mediumtext)
BEGIN
    ...
    SELECT COUNT(DISTINCT topic_id) AS total_count 
	 ...
	 WHERE poster_id = @user_id
       AND NOT FIND_IN_SET (forum_id, excluded_forums);
END